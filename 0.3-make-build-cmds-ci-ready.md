...
# CI & SonarCloud Setup – Detailed Recap (Post-Liquibase/Test Fixes)

This recap covers **everything we did after fixing the Spring Boot test failures**, up through the current SonarCloud wiring issue. This is written so future-you (or a teammate) can understand *why* each decision was made, not just *what* we did.

---

## 1. Why CI Started Failing After Tests Were Fixed

Once tests passed locally and with Testcontainers, we moved to **CI quality gates**. This introduced a new class of failures that were **not application bugs**, but **tooling compatibility and configuration issues**.

Key transition:
- From “can the app build and test?”
- To “can the app be analyzed and governed automatically?”

This is where SonarCloud entered the picture.

---

## 2. Sonar Gradle Plugin vs Gradle 9 Compatibility
...
```
NoSuchMethodError: DslObject.getConvention()
```

### Root Cause

* You are using Gradle 9.x

* Older Sonar Gradle plugins (4.x, 6.0.x) still used deprecated Gradle APIs

* Gradle 9 removed those APIs entirely

This is not a configuration error. It is a **binary incompatibility.**

Fix

Upgrade to a Sonar Gradle plugin that explicitly supports Gradle 9.
...
* The getConvention() error disappeared

* The Sonar task could now execute normally


## 4. “Project Key Cannot Be Empty” Error

### Symptom

After fixing the plugin version, Sonar failed with:
...
The Sonar Gradle plugin configuration was not receiving the required properties from environment variables in CI.
...
* Environment variables were not set yet

* Sonar received an empty project key

This was expected and correct behavior.

### Key Insight

This failure confirmed:

* The build was correctly reading environment variables

* Sonar was enforcing explicit configuration

* There was no hidden or hard-coded state

## 5. SonarCloud UI Confusion (“You’re Done” Screen)

### What Happened

SonarCloud UI displayed a message similar to:

> “You’re done. Run your first analysis…”

But:

- No analysis had run yet
- The project key was not obviously visible
- The UI appeared incomplete

### Reality

At this stage:

- SonarCloud had created the project
- It was waiting for the first analysis
- The project key already existed, but was not prominently shown


## 9. Current State (As of Now)

### What Is Working

- `./gradlew clean test` passes locally
- Tests pass in CI
- Testcontainers + Liquibase are correctly integrated
- Sonar Gradle plugin is compatible with Gradle 9
- SonarCloud project exists
- GitHub secrets exist

## 10. Why This Phase Matters

Everything in this phase is **infrastructure work**:

- It does not add features
- It prevents future chaos
- It ensures:
    - consistent builds
    - reproducible analysis
    - enterprise-grade hygiene

Most teams skip or rush this and pay for it later.
...
We will **not move on to 0.3.4 (FOSSA)** until:

- SonarCloud analysis passes in GitHub Actions
- The “Project not found” error is resolved
- The debug step confirms correct env wiring

Once Sonar is green, dependency scanning will be straightforward.
