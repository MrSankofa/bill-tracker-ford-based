...
# 0.4.1.10 – 0.4.1.21 (Prep) Recap
**Tekton + Gradle + Testcontainers Alignment**

This section documents the debugging journey and final resolution that allowed our Tekton-based CI pipeline to run backend tests successfully **without Testcontainers**, while preserving Testcontainers-based tests for local and GitHub Actions usage (Pattern A).

---

## Context Recap

- We are running **Gradle + Spring Boot (Java 21)** tests inside **Tekton on Kubernetes (Docker Desktop)**.
- Some tests use **Testcontainers**, which requires Docker access.
- Tekton pods **do not have Docker access**, so Testcontainers tests must be excluded in Tekton.
- Long-term strategy chosen:  
  **Pattern A**
   - Testcontainers run:
      - Locally
      - In GitHub Actions (Docker-enabled)
   - Testcontainers are **excluded in Tekton**

---

## Initial Symptoms

- Tekton PipelineRuns consistently failed with errors like:

```java.lang.IllegalStateException at DockerClientProviderStrategy```

- This indicated Testcontainers was still attempting to start Docker-based containers.
- Even after passing `-PexcludeTags=testcontainers`, tests were **not being excluded**.

---

## Key Investigation Steps

### 1. Verified Tekton Was Using the Correct Branch and Params

From Tekton logs:
- Repo URL: correct
- Branch: `041-tekton-foundations`
- Gradle command included:

```-PexcludeTags=testcontainers```


So the **inputs were correct**.

---

### 2. Determined the Real Workspace Path

Using a debug pod mounted to the same PVC:
- Repo root was at:

```/ws/```

```-Backend module path: /ws/backend```


This confirmed Tekton was cloning the correct repo contents.

---

### 3. Confirmed Tests Were Properly Tagged

Using grep inside the workspace:
- Found `@Tag("testcontainers")` on:
- `LiquibaseWiringTest`
- `BillTrackerFordBasedBackendApplicationTests`
...
✅ Tests were correctly tagged.

---

### 4. Found the Root Cause (Critical Discovery)

In `backend/build.gradle`:

```groovy
// Usage: ./gradlew test -PexcludeTags=testcontainers
// if (project.hasProperty('excludeTags')) {
//     excludeTags(project.property('excludeTags').toString())
// }
```

- The `excludeTags` logic was **commented out**!

- Therefore, even though we passed `-PexcludeTags=testcontainers`, it was **not being applied**.

Result:
* Gradle accepted -PexcludeTags=testcontainers but did nothing with it.
* Testcontainers tests ran anyway, causing failures.

### The Fix
Uncomment the `excludeTags` logic in `backend/build.gradle`:
...
Key points:
* Ensured `excludeTags` was applied within the `test` task configuration.
* Explicitly called `useJUnitPlatform` to configure JUnit 5.
* Properly wires the gradle project properties to Junit tag filtering.


### Local Verification (Critical Step)

Ran locally:

```./gradlew clean test -PexcludeTags=testcontainers```

- Verified Testcontainers tests were excluded.
Result:
```shell
...
This confirmed:
* tag exclusion logic worked as intended.
* No Testcontainers tests ran.

Note:
-- info | grep excluded is not reliable for confirming exclusions because it refers to gradle tasks, not test cases.

### Final Tekton Run

Updated Tekton PipelineRun to use:

```./gradlew clean test -Dspring.profiles.active=test -PexcludeTags=testcontainers```
...
2. Find the new gradle pod:

```shell
kubectl get pods | grep gradle-test
```

3. Pull logs from the gradle-test step:
...
### Expected Outcome
- Tekton PipelineRun completes **successfully**.
- Logs show:
- ```BUILD SUCCESSFUL```

### One more reminder (So you don't get ghost failures)

you must push the changes to `backend/build.gradle` that uncomment the `excludeTags` logic.
and change the branch to `041-tekton-foundations` in the PipelineRun definition. Since Tekton clones the repo fresh each time, it needs to see those changes in the branch it's using.
if you did not push those changes, the PipelineRun will continue to fail with Testcontainers errors.

Run this locally to confirm you pushed:
...
We are now correctly positioned to proceed with:
* 0.4.1.6.1 - cleanup and hardening
* 0.4.2 - Real Tekton Pipelines + Triggers
* Frontend Cypress track