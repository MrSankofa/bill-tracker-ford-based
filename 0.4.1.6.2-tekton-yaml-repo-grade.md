...
| sed "s/^  git-revision: main$/  git-revision: ${BRANCH}/" \
| kubectl create -f -
```
This command dynamically sets the `git-revision` parameter to your current branch name when creating the PipelineRun, allowing you to run tests against the branch you're working on without modifying the YAML file directly.

Why this works
* The `--dry-run=client -o yaml` option generates the YAML for the PipelineRun without actually creating it in the cluster.
* The `sed` command replaces the default `git-revision: main` line with the
* kubectl create -f - command reads the modified YAML from standard input and creates the PipelineRun in the cluster.

No file edits are necessary, keeping your YAML repo clean and consistent.
